...
BEGIN_MESSAGE_MAP(CSerialPortMonDlg, CDialog)
	...
	ON_WM_SIZING()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BTN_FONT, &CSerialPortMonDlg::OnClickedBtnSetFont)
END_MESSAGE_MAP()
...
BOOL CSerialPortMonDlg::OnInitDialog()
{
	...
	_fontFixedsys.CreatePointFont(120,_T("Fixedsys"));   
	_editMessage.SetFont(&_fontFixedsys);

	_editInput.SetFont(&_fontFixedsys);

	// 버젼 정보와 홈페이지 하이퍼링크 표시
//	SetDlgItemText (IDC_STATIC_VERSION, GetVersionInfo(NULL, "ProductVersion")); // <-- 문제있슴.
	SetDlgItemText(IDC_STATIC_VERSION, "Version 1.2 (Updated: 2015.08.07)");
...
void CSerialPortMonDlg::WordWrap ()
{
	...
	_editMessage.SetFont(&_fontFixedsys);			// 2025.08.07 기존 폰트설정
...
void CSerialPortMonDlg::OnClickedBtnSetFont()
{
	// TODO: Add your control notification handler code here
	LOGFONT lf = { 0 };
	_fontFixedsys.GetLogFont(&lf);
	CFontDialog dlg(&lf);

	if (dlg.DoModal() == IDOK)
	{
		// 기존 폰트 제거
		_fontFixedsys.DeleteObject();

		// 선택된 LOGFONT로 새 폰트 생성
		dlg.GetCurrentFont(&lf);
		_fontFixedsys.CreateFontIndirect(&lf);
